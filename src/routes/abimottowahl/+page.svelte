  ...
  ' alles Begann mit einer Tüte'
	...
	onMount(async () => {
		...
		onAuthStateChanged(auth, async (firebaseUser) => {
			if (firebaseUser) {
				checkEmail(firebaseUser.email!, names)
				isLoggedIn = true;
			} else {
				isLoggedIn = false;
			}
	...
	async function register() {
		...
		email = benutzer + '@ewoxvotesystem.com';
		console.log(email);
		try {
...
{:else}
	<div class="login-container">
		...
		<div class="login-box">
			...
			<input type="text" bind:value={password} placeholder="Zugangscode" class="login-input" />
